...
@api_view(["GET"])
def getCostumerRange(request, tk, start, end):
...
def deletecostumer(request, pk, tk):
    if check_auth(tk) == True:
        ...
        return JsonResponse(
            "{'request' : 'failed', 'User' : 'User token invalid'}", safe=False
        )
...
def getcollectiondetail(request, pk):
    user_lists = Collectionlist.objects.filter(costumer_id=pk).values(
        "id", "date", "ammount", "costumer_id", "costumer_name"
    )
...
def checkDuplicate(data):
    ...
    CollectionData = Collectionlist.objects.filter(date=date, costumer_id=id)
...
def getcostumerlastCollection(request, tk, pk):
    if check_auth(tk) == True:
        data = (
            Collectionlist.objects.filter(costumer_id=pk)
            .filter(ammount__gt=0)
            .order_by("-id")[0]
        )
        serializer = Collectionlistserializer(data, many=False)
        return JsonResponse(serializer.data, safe=False)
...
def getcollectionbydate(request, tk, pk):
    collection = Collectionlist.objects.filter(date=pk).values(
        "id", "date", "ammount", "costumer_id", "costumer_name"
    )
...
def CostumerCollectionbyDate(pk, date):
    x = 0
    collection = Collectionlist.objects.filter(date=date, costumer_id=pk).values(
        "id", "date", "ammount", "costumer_id", "costumer_name"
    )
    serializer = Collectionlistserializer(collection, many=True)
...
def getCostumerCollectionbyDate(request, tk, pk, date):
    collection = Collectionlist.objects.filter(date=date, costumer_id=pk)
    serializer = Collectionlistserializer(collection, many=True)
    return JsonResponse(serializer.data, safe=False)
